...
class Mkdir extends Command {
  ...
  Future<ProcessResult> execute({
    ...
    if (arguments.isEmpty) {
      errorln('Usage: mkdir <directory_name>');
    ...
    for (String dirPath in directoriesToCreate) {
      try {
        ...
        if (await dir.exists()) {
          if (!recursive) {
            errorln("mkdir: cannot create directory '$dirPath': File exists");
        ...
        writeln('Created directory: $dirPath');
      } catch (e) {
        errorln('Failed to create directory $dirPath: $e');
